    ...
    <x-app.container class="lg:space-y-6">
        <div class="flex items-center justify-between pb-5 border-b border-zinc-200 dark:border-zinc-800">
            <x-app.heading
                ...
                description="Customise your headless backend sequence, wait times, intents, and action paths."
                :border="false"
            />
        ...
        <div class="mt-6 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 shadow-sm">
            <form action="{{ route('automations.update', $automation->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-bold text-zinc-700 dark:text-zinc-300 mb-1">Workflow Name</label>
                        <input type="text" name="name" value="{{ old('name', $automation->name) }}" class="w-full border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-850 rounded-lg p-2.5 text-sm" placeholder="e.g. Inbound Reply Sequence" required>
                    </div>
                    <div>
                        ...
                        <select name="type" class="w-full border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-850 rounded-lg p-2.5 text-sm" required>
                            <option value="action" {{ old('type', $automation->type) === 'action' ? 'selected' : '' }}>Action (e.g. outreach queue sequence)</option>
                            <option value="trigger" {{ old('type', $automation->type) === 'trigger' ? 'selected' : '' }}>Trigger (e.g. smart reply handler)</option>
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="workflow_definition" rows="18" class="w-full font-mono text-xs border border-zinc-300 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900 rounded-lg p-3" required>{{ old('workflow_definition', json_encode($automation->workflow_definition, JSON_PRETTY_PRINT)) }}</textarea>
                </div>
                ...
                <div class="pt-4 flex justify-end space-x-3 border-t border-zinc-200 dark:border-zinc-800">
                    ...
                    <button type="submit" class="px-5 py-2 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg shadow">Save Changes</button>
                </div>
            </form>
        </div>
    </x-app.container>
