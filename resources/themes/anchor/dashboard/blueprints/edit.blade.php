    ...
    <x-app.container class="lg:space-y-6">
        <div class="pb-5 border-b border-zinc-200 dark:border-zinc-800">
            <x-app.heading
                ...
                description="Modify name, details, max attempts, or add/remove/reorder steps in this sequence blueprint."
                :border="false"
            />
        </div>

        <div class="mt-6 max-w-2xl bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 shadow-sm">
            <form action="{{ route('blueprints.update', $blueprint->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="max_attempts" id="max_attempts" required value="{{ old('max_attempts', $blueprint->max_attempts) }}" min="1" class="mt-1 block w-full rounded-lg border border-zinc-300 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 text-sm py-2 px-3 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
    ...
    <script type="text/javascript">
        ...
        document.addEventListener('DOMContentLoaded', () => {
            @foreach($blueprint->steps as $step)
                addStepRow('{{ $step->template_id }}', '{{ $step->wait_days }}');
            @endforeach

            // If blueprint has no steps, append an empty one
            if (document.getElementById('steps-list').children.length === 0) {
                addStepRow();
            }
        });
    </script>
